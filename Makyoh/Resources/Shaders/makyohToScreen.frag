...
layout (binding = 0) uniform sampler2D normalHeightTexture;
layout (binding = 1) uniform sampler2D irradianceTexture;
...
void main()
{
    ...
    for (int x = 0; x < 5; x++) {
        for (int y = 0; y < 5; y++) {
            IA += texture(irradianceTexture, texCoords + (vec2(x, y) - vec2(2, 2)) / textureSize(irradianceTexture, 0)).ra;
        }
    }   
    ...
    */
    ...
    if (visHeight == 0) {   // Render power density
        ...
        float h = texture(normalHeightTexture, texCoords).w;
        FragColor = vec4(h.xxx, 1.0);
    }
    else if (visHeight == 2) {  // Render normal
        FragColor = vec4(texture(normalHeightTexture, texCoords).xyz, 1.0);
    }    
}