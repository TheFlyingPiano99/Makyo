...
*/
	...
	Scene* SceneFactory::CreateMakyohScene(unsigned int _contextW, unsigned int _contextH)
	{
		auto* scene = LoadSceneFromFile(AssetFolderPathManager::getInstance()->getScenesFolderPath()
			.append("magicMirrorScene02_gauss.json"));
		
		auto* makyohCanvas = Allocator::New<MakioSim::MakyoCanvas>();
		auto* normHeightProgram = scene->GetShaderProgramByName("mirrorNormalHeightShader");
		auto* canvasProjectionProgram = scene->GetShaderProgramByName("canvasProjectionShader");
		makyohCanvas->Init(normHeightProgram, canvasProjectionProgram);
		scene->GetSceneObjectByName("Dummy makyoh obj")->AddComponent(makyohCanvas);
		scene->GetSceneObjectByName("Dummy makyoh obj")->SetIsVisible(false);
		scene->GetSceneObjectByName("Dummy makyoh normal-height obj")->SetIsVisible(false);
		...
		if (inputFile.is_open()) {
			try {
				...
				for (auto& shaderData : jsonData["shaders"]) {
					...
					std::cout << "Uniforms count at reading: " << shaderData["uniforms"].size() << "\n";
					for (auto& uniformData : shaderData["uniforms"]) {
						...
						if ("float" == typeStr) {
							...
							std::cout << "Float uniform: " << uniVar->GetName() << "\n";
							shader->BindUniformVariable(uniVar);
							std::cout << "Address from shader: " << shader->GetUniformVariable(name) << "\n";
						}
